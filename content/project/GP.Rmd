...
```{r}
#Obtaining a numeric vector for time. Maintaining the units of measure at 1
...
where $\sigma^2$ is the estimated variance accounted for by the function $k$ and $l$ is a length scale parameter that governs the decay rate. Smaller estimated values for $l$ indiciate a faster decay rate in the ability of the function account for the covariance between two time points as a function of time. 

This model, along with its forecasting function are defined in Stan code below: 
...
Unfortunately, the sampling algorithm that generates the Bayesian estimates is not parallelizable. Until `Stan` and `rstan` can run using graphics chips' architecture (which has been buzzed about around the [Stan ether](http://discourse.mc-stan.org/t/stan-on-the-gpu/326)), model run time is going to be the biggest downside. Still, 23 minutes is not that long to wait if accuracy matters. 
...
Still, it is impossible to know which of these models is better, a methodological question I may tackle in greater detail when I have some time. For now, the main takeaway is that Gaussian process models may represent a useful approach to the age-old problems of forecasting and imputation, a fact I plan to exploit in some of my signal processing work. 