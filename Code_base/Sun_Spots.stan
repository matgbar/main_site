functions{
	...
	matrix main_GP(
		...
		real mu_Hz){
					...
					for(i in 1:Nx){
						for(j in 1:Ny){
							K2[i, j] = alpha2*exp(-2*square(sin(pi()*fabs(x[i]-y[j])*mu_Hz))/square(rho2))*
	...
	Ko = main_GP(No, xo, No, xo, a1, a2, r1, r2, r3, Hz);
	for(n in 1:No) Ko[n,n] += sn;
		
	//--------------------------------------------------------------------
	//kernel for predicted data
	Kp = main_GP(Np, xp, Np, xp,  a1, a2, r1, r2, r3, Hz);
	for(n in 1:Np) Kp[n,n] += sn;
		
	//--------------------------------------------------------------------
	//kernel for observed and predicted cross 
	Kop = main_GP(No, xo, Np, xp,  a1, a2, r1, r2, r3, Hz);
...
parameters {
	real<lower=0> a1;
	real<lower=0> a2;
	real<lower=0> r1;		//Set after some preliminary modeling
	real<lower=0> r2;
	real<lower=0> r3;
...
model{ 
	...
	Sigma = main_GP(N1, X, N1, X, a1, a2, r1, r2, r3, Hz);
	...
	a1 ~ student_t(3,0,5);		
	a2 ~ student_t(3,0,5);
	//incorporate minimum and maximum distances - use half student_t distribution
	r1 ~ student_t(3,0,5);	
	r2 ~ student_t(3,0,5);
	r3 ~ student_t(3,0,5);	
	Hz ~ normal(mu_Hz, 10);
	sigma_sq ~ student_t(3,0,5);
}

generated quantities {
	vector[N2] Ypred = post_pred_rng(a1, a2, r1, r2, r3, sigma_sq, Hz, N1, X, N2, Xp, Y);
}
